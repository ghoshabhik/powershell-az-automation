## Powershell Script to add user to an AD group 
## Input Parameter: CSV File with list of users; File name is expected as group name
## Run Script Example: ./AddUserToADGroup.ps1 <GROUP_NAME>.csv
...
Function Check-AdUserExistsInGroup(){
    ...
    if($user){ 
        Write-Host "$userPrincipalName : already exists in group: $groupId" -ForegroundColor Red
        return $true
    } else {
        Write-Host "$userPrincipalName : does not exist in group: $groupId" -ForegroundColor Green
        return $false
    }
}
...
foreach ($line in $inputFile)
{
    ...
    if($groupId){ 
        ...
        if($userId){ 
            ...
            if(-not (Check-AdUserExistsInGroup $inputUser $groupId)){   ## ---Check if user is already in the group ---
                try{
                    Add-AzADGroupMember -TargetGroupObjectId $groupId -MemberObjectId $userId  ## ---Add User to the Group---
                    Write-Host "$inputUser : Added to AD Group : $inputGroupName" -ForegroundColor Green
                    }
                catch {
